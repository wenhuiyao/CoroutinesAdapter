...
interface ConsumerOperator<T, R> : ProducerBuilder<T, R> {
     ...
     */
    fun <M> transform(action: ConsumeAction<R, M>): ConsumerOperator<T, M> = transform(CoroutineContexts.BACKGROUND, action)

    /**
     * Transform a source from type T to type R
     *
     * NOTE: currently, [CoroutineContexts.NON_CANCELLABLE] is not supported
     */
    fun <M> transform(context: CoroutineContexts, action: ConsumeAction<R, M>): ConsumerOperator<T, M>

    /**
     * Operate on the item, by default it is running on the background thread
     */
    fun operate(action: ParametrizedAction<R>): ConsumerOperator<T, R> = operate(CoroutineContexts.BACKGROUND, action)

    fun operate(context: CoroutineContexts, action: ParametrizedAction<R>): ConsumerOperator<T, R>
...
private class ProducerConsumer<T, R>(private val consumer: Consumer<T>,
    ...
    override fun <M> transform(context: CoroutineContexts, action: ConsumeAction<R, M>): ConsumerOperator<T, M> {
        ensureContextCancellable(context)
        val newTransformer = TransformerImpl(executor, action, context.context)
        return ProducerConsumer(consumer, newTransformer)
    }

    override fun operate(context: CoroutineContexts, action: ParametrizedAction<R>): ConsumerOperator<T, R> {
        ensureContextCancellable(context)
        val newTransformer = OperatorImpl(executor, action, context.context)
        return ProducerConsumer(consumer, newTransformer)
    }
