...
interface Operator<T> : Worker<T> {
     ...
     */
    fun <R> transform(action: TransformAction<T, R>): Operator<R> = transform(CONTEXT_BG, action)
     ...
     */
    fun <R> transform(context: CoroutineContext, action: TransformAction<T, R>): Operator<R>

    /**
     * Operate on the item, by default it is running on the background
     */
    fun operate(action: ParametrizedAction<T>): Operator<T> = operate(CONTEXT_BG, action)

    fun operate(context: CoroutineContext, action: ParametrizedAction<T>): Operator<T>
}
